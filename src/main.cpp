...
uint256 hashGenesisBlock("0x747bb19e3489c4e8cbef01cf52485311e7ef95f8e4fcf42e3bac96341bf2abdb");
...
const string strMessageMagic = "Gold Cryptocurrency Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 3) {
        nSubsidy = 60000000 * COIN; //5% premine
    } else if(nHeight <= 19440) {
        nSubsidy = 5 * COIN; //prevents instamine
    } else if(nHeight <= 630720) {
        nSubsidy = 40 * COIN; //40 coins per block for the first 4 years
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 200 * 3; // Gold Cryptocurrency diff retarget: every 600 seconds
static const int64 nTargetSpacing = 200; // Gold Cryptocurrency confirmation time: every 200 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal gold miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Gold Cryptocurrency", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6a034aad5f82e3b48575defa49522147821f0e69c983293e710620b2cdab4fec");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Truth, like gold, is to be obtained not by its growth, but by washing away from it all that is not gold.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501171422; //epochtime = Thu, 27 Jul 2017 16:03:41 GMT
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 766689;

        if (fTestNet)
        {
            block.nTime    = 1501034967;
            block.nNonce   = 497001;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x86bd08e73ca26cc10def018c7f69c2594ab07d37ae4506dbde0203cad96e374d"));
